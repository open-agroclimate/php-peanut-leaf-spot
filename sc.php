<?php
  ...
  include('conn.php');
	$result_stations = mysql_query("SELECT * FROM stations2 where state='SC' order by name") or die (mysql_error());
	while ($st_name = mysql_fetch_array($result_stations)) {
	  $stations[] = $st_name['name'].", SC";
	  $lat[] = $st_name['lat'];
	  $long[] = $st_name['long'];
	...
	$current_risk = mysql_query("SELECT * FROM stations2,stations2_current_risk where state='SC' and stations2.nst_abbr=stations2_current_risk.nst_abbr order by name") or die (mysql_error());
...
<script type="text/javascript">
 ...
 /* for( y=0; y<stationsName.length; y++){ 	
  document.write( stationsName[y] + "<br>" );	
 }*/
 ...
 var p;  
 lat = lats.split(":#:");
 /* for( p=0; p<lat.length; p++){ 	
  document.write( lat[p] + "<br>" );	
 } */
 ...
 var z;  
 nst = nsts.split(":#:");
 /* for( z=0; z<nst.length; z++){ 	
  document.write( nst[z] + "<br>" );	
 } */
...
stationDescription.push("<div class='infowindow'><div class='stname'>"+stationsName[x]+"</div><b>"+risks[x]+"</b><br><a href='risks2.php?nst="+nst[x]+"'>Click here for more Info</div>");
 ...
 center: new google.maps.LatLng(33.5, -80.75),
 };
