<?php
  ...
  $result_stations = mysql_query("SELECT * FROM stations1 where state='GA' order by name") or die (mysql_error());
	while ($st_name = mysql_fetch_array($result_stations)) {
	  $stations[] = $st_name['name'].", GA";
	  $lat[] = $st_name['lat'];
	  $long[] = $st_name['long'];
	}
	...
	$result_risk = mysql_query("SELECT * FROM stations1_risks where state='GA' order by name") or die (mysql_error());
...
stationDescription.push("<div class='infowindow'><div class='stname'>"+stationsName[x]+"</div><b>"+risks[x]+"</b><br><a href='risks1.php?id="+id[x]+"'>Click here for more Info</div>");
 ...
 center: new google.maps.LatLng(32.5, -83),
 };
